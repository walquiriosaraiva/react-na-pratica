...
export function App() {
  ...
  const { data: tagsResponse, isLoading, isFetching } = useQuery<TagResponse>({
    queryKey: ['get-tags', rowsPage, urlFilter, page],
    queryFn: async () => {
      const response = await fetch(`http://localhost:3333/tags?_page=${page}&_per_page=${rowsPage}&title=${urlFilter}`)
  ...
  const handleSelectedValueChange = (rowsPage: number) => {
  ...
  return (
    <div className="py-10 space-y-8">
      ...
      <main className="max-w-6xl mx-auto space-y-5">
        ...
        {tagsResponse && <Pagination pages={tagsResponse.pages} items={tagsResponse.items} page={page} rowsPage={rowsPage} onRowsPage={handleSelectedValueChange} />}
      </main>
    </div>
  )
}
